...
class Cache_File extends Cache implements Cache_GarbageCollect {
	...
	protected function __construct(array $config)
	{
		...
		try
		{
			$directory = isset($this->_config['cache_dir']) ? $this->_config['cache_dir'] : sys_get_temp_dir() . DIRECTORY_SEPARATOR . 'cache';
		...
		{
			throw new Cache_Exception('Unable to read from the cache directory ' . $this->_cache_dir->getRealPath() );
		}

		// Check the write status of the directory
		if ( ! $this->_cache_dir->isWritable())
		{
			throw new Cache_Exception('Unable to write to the cache directory ' . $this->_cache_dir->getRealPath() );
		}
	}
	...
	{
		return $this->_cache_dir->getPathname().DIRECTORY_SEPARATOR.$filename[0].$filename[1].DIRECTORY_SEPARATOR;
	}
