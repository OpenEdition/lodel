...
class Cache_Sqlite extends Cache implements Cache_Tagging, Cache_GarbageCollect {
	...
	{
		parent::__construct($config[$config['driver']]);

		$database = isset($this->_config['database']) ? $this->_config['database'] : NULL;
		...
		if (0 == count($result))
		{
			$database_schema = isset($this->_config['schema']) ? $this->_config['schema'] : NULL;
			...
			try
			{
				...
				$this->_db->query(isset($this->_config['schema']) ? $this->_config['schema'] : NULL);
			}
	...
	public function set_with_tags($id, $data, $lifetime = NULL, array $tags = NULL)
	{
		...
		if ($lifetime === NULL)
		{
			$lifetime = isset($this->_config['default_expire']) ? $this->_config['default_expire'] + time() : Cache::DEFAULT_EXPIRE + time();
		}
		else
		{
