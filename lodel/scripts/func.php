...
/**
 *   Extrait toutes les variables pass�s par la m�hode post puis les stocke dans 
 *   le tableau $context
 */
...
function clean_request_variable(&$var)
{
	...
	foreach(array_keys($var) as $k) {
		...
		$var = str_replace(array("\n", "&nbsp;"), array("", "�240"), $filter->process(trim($var)));
  }
}
...
{
	global $db;
	// D�errouilles toutes les tables v�ouill�s
	// fonction lock_write()
...
{
	global $db;
  // V�ouille toutes les tables MySQL en �riture
  $list = func_get_args();
	if (!defined("DONTUSELOCKTABLES") || !DONTUSELOCKTABLES)
...
/**
 * retourne le chemin complet vers une entit� *
 * @param integer $id identifiant num�ique de l'entit� * @param string $urltype le type d'url utilis�(path,querystring)
 * @return string le chemin
 * @since fonction ajout� en 0.8
 */
function getPath($id, $urltype,$base='index')
{
	$urltype = 'querystring'; //la version actuelle de lodel ne g�e que le type path
...
/**
 * V�ifie que le r�ertoire $dir, un r�ertoire de docannexe existe. Dans le cas
 * contraire le cr�
 *
 * @param string $dir le nom du r�ertoire
 */
 ...
 *
 * Cette fonction est insir� de Dotclear et de
 * http://w3.org/International/questions/qa-forms-utf-8.html.
 *
 * @param string $string la cha�e �tester
 * @return le r�ultat de la fonction preg_match c'est �dire false si la cha�e n'est pas en
 * UTF8
 */
...
/**
 * Transforme une chaine de caract�e UTF8 en minuscules d�accentu�s
 *
 * Cette fonction prends en entr� une cha�e en UTF8 et donne en sortie une cha�e
 * o les accents ont ��remplac� par leur �uivalent d�accentu� De plus les caract�es
 * sont mis en minuscules et les espaces en d�ut et fin de cha�e sont enlev�.
 *
 * Cette fonction est utilis� pour les entr�s d'index ainsi que dans le moteur de recherche
 * et pour le calcul des identifiants litt�aux.
 *
 * @param string $text le texte �passer en entr�
 * @return le texte transform�en minuscule
 */
function makeSortKey($text)
{
	$text = strip_tags($text);
	//remplacement des caract�es accentues en UTF8
 ...
 * @param string $indenter les caract�es �utiliser pour l'indentation. Par d�aut deux espaces.
 ...
 * @param string $indent les caract�es �utiliser pour l'indentation. Par d�aut deux espaces.
 * @return le code indent�proprement :)
 */
...
/**
 * R�up�ation des champs g��iques dc.* associ� aux entit�
 *
 * @param integer $id identifiant num�ique de l'entit�dont on veut r�up�er un champ dc
 * @param string $dcfield le nom du champ �r�up�er (sans le dc.devant). Ex : .'description' pour 'dc.description'
 * @return le contenu du champ pass�dans le param�re $dcfield
 */
...
// Tente de r�up�er la liste des locales du syst�e dans un tableau
...
/**
 * R�up�e les champs g��iques d�inis pour une entit� *
 * Stocke les champs g��iques d�inis pour une entit�dans un sous tableau de $context : generic
 *
...
function getgenericfields(&$context)
{
	...
	//Contruit le tableau des champs g��iques avec leur valeur
	...
	//R�up�e maintenant les valeurs des champs g��iques des entr�s d'index associ�s et des personnes associ�s
	...
	//Retrouve les valeurs des entr�s en utilisant le g_name de la table entries
	if(count($fields) > 0) {
	...
	return $context; // pas n�essaire le context est pass�par r��ence
}
