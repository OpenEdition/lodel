...
function checkstring(& $text, $error = 0)
{
	...
	if (!xml_parse($xml_parser, $text))	{
		if (!$error) {
			...
			echo "L'error se situe avant la zone rouge. Elle peut tre due  une error bien au dessus la ligne donne par le parser<br>";
