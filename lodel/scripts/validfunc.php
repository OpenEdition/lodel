<?
/**
 * Fichier utilitaire pour g�er la validation des champs
 *
 * PHP version 4
 *
 ...
 *
 * <p>Validation des caract�es autoris� dans les champs suivant leur type
 * leur nom, et le texte contenu. Par exemple si on a un champ de type email, il faut
 * v�ifier que l'adresse mail est bien form�. Idem pour un champ de type url. Cela appliqu� * �tous les types de champs g�� par Lodel (cf. fichier fieldfunc.php)</p>
 *
 * @param string $&text le texte �valider. Pass�par r��ence.
 * @param string $type le type du champ �valider.
 * @param string $default la valeur par d�aut �valider (si le texte est vide). Est vide par d�aut
 * @param string $name le nom du champ
 * @param string $usedata indique si le context utilise le sous tableau data pour stocker les donn�s
...
function validfield(&$text, $type, $default = "", $name = "", $usedata = "")
{
	...
	case 'date' :
	case 'datetime' :
	case 'time' : //v�ification des champs date, time et datetime
