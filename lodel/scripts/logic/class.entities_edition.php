	...
	public function viewAction(&$context, &$error)
	{
		...
		if(!function_exists('loop_entries_in_entities'))
		{
			function loop_entries_in_entities($context, $funcname) 
			{
				...
				$votype = DAO::getDAO("entrytypes")->getById($idtype, "id,sort,flat,upd");
				if (!$votype) {
				...
				if (isset($context[$var][$idtype]) && is_array($context[$var][$idtype])) {
					foreach ($context[$var][$idtype] as $entry) {
		...
		if(!function_exists('loop_entries_in_entities_rec'))
		{
			function loop_entries_in_entities_rec ($context, $funcname, &$votype, &$checkarr) 
			{
				...
				$id = @$context['data']['identity'];

				// get the entries
				$result = $db->execute(lq("SELECT * FROM #_TP_entries WHERE idtype='". $votype->id. "' AND idparent='". $context['id']. "' AND status>-64 ORDER BY ". $votype->sort)) 
