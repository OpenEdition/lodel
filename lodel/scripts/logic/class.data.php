 ...
 * @copyright 2004, Ghislain Picard, Marin Dacos, Luc Santeramo, Anne Gentil-Beccot, Bruno C�ou
...
/**
 * Classe de logique permettant de g�er les backup et import de donn�s et de ME
 * 
 * @package lodel/logic
 * @author Jean Lamy
 * @copyright 2001-2002, Ghislain Picard, Marin Dacos
...
class DataLogic
{
	...
	/**
	 * Prefix du fichier (pour l'import de ME et l'import de donn�s)
	 * @var string
	 */
	var $filePrefix;

	/**
	 * Expression r�uli�e pour filtrer les fichiers pour un import
	 * @var string
	 */
	var $fileRegexp;
	 ...
	 * Constructeur
	 *
	 * Interdit l'acc� aux utilisateurs qui ne sont pas ADMIN
	 */
	function DataLogic()
	{
	...
	/**
	 * Importation des donn�s
	 *
	 * Cette fonction importe les donn�s issus d'un backup de lodel : le dump SQL, les fichiers associ� (si ils ont ��sauvegard�).
	 *
	 * @param array $context le contexte pass�par r��ence
	 * @param array $error les �entuelles erreur, pass�s par r��ence
	 */
	...
	/**
	 * Sauvegarde des donn�s
	 *
	 * Fait un dump de la base de donn�s du site et si indiqu�sauve aussi les fichiers annexes et source.
	 *
	 * @param array $context le contexte pass�par r��ence
	 * @param array $error les �entuelles erreur, pass�s par r��ence
	 */
	...
	/**
	 * Backup global des donn�s. Seulement autoris�pour un admin lodel
	 *
	 * Cela cr� un backup de la base principale mais aussi de tous les sites
	 *
	 * @param array $context le contexte pass�par r��ence
	 * @param array $error les �entuelles erreur, pass�s par r��ence
	 * @return le nom du template utilis�pour cette action : backup
	 * @todo Trouver une alternative �la commande syst�e tar
	 */
	...
	/**
	 * Importation du mod�e �itorial
	 *
	 * Importe les donn�s contenu dans un fichier ZIP de sauvegarde du ME
	 *
	 * @param array $context le contexte pass�par r��ence
	 * @param array $error les �entuelles erreur, pass�s par r��ence
	 * @return le nom du template utilis�pour cette action : importmodel
	 */
	function importmodelAction(&$context, &$error)
	{
		//V�ifie que l'on peut bien faire cet import
		$context['importdir'] = $GLOBALS['importdir']; //cherche le rep d'import d�ini dans la conf
		...
		if ($file && $context['delete']) {// extra check. Need more ?
			...
			if (!$err) {
				if ($context['frominstall']) { // si on vient de l'install redirige vers la page d'�ition
	...
	/**
	 * Sauvegarde du mod�e �itorial
	 *
	 * Sauve les tables du ME dans un dump SQL (table lodel + table cr�s). Si demand� inclue
	 * aussi les templates, les css, les images et les scripts javascript. Le fichier cr� est
	 * de la forme <em>model-site-date.zip</em>.
	 *
	 * @param array $context le contexte pass�par r��ence
	 * @param array $error les �entuelles erreur, pass�s par r��ence
	 * @return le nom du template utilis�pour cette action : backupmodel
	 */
	function backupmodelAction(&$context, &$error)
	{
		...
		if ($context['backup']) {
			...
			// R�up�e la liste des tables de classe �sauver.
			$dao = &getDAO('classes');
	 ...
	 * @param string $outfile le fichier dans lequel �rire le dump SQL
	 * @param resource $fh le descripteur de fichier (par d�aut 0)
	 ...
	 *
	 * @todo v�ifier que cette fonction ne prends pas trop de place en m�oire.
	 * @access private
	 * @param string $url le fichier SQL
	 * @param boolean $ignoreerrors. false par d�aut
	...
	/**
	 * V�ifie les fichiers CACHE et .htaccess et recr�les .htaccess.
	 *
	 * @param array $context le contexte pass�par r��ence.
	 */
	...
	/**
	 * Cr� un fichier ZIP du ME contenant le fichier SQL et �entuellement les r�ertoires
	 * images, css, js et tpl.
	 *
	 * @access private
	 * @param string $sqlfile le fichier dump SQL
	 * @param array $dirs la liste des r�ertoires �inclure.
	...
	function _backupME($sqlfile, $dirs = array())
	{
		...
		// Cherche si les r�ertoire �zipper contiennent bien des fichiers
		$zipdirs = array ();
		foreach ($dirs as $dir)	{
			if (!file_exists(SITEROOT. $dir))
				continue;
	 ...
	 *
	 * V�ifie si le site est vide, pour permettre l'import d'un ME
	 *
	 * @access private
	 ...
	 *
	 * 
	 * @param string $footprint le prefix qui doit �re contenu dans le nom du fichier
	 * @param array $context le contexte pass�par r��ence
	 * @param string $ext l'extension du fichier, par d�aut .zip
	...
	function _extractImport(&$context)
	{
		$archive = $_FILES['archive']['tmp_name'];
		$context['error_upload'] = $_FILES['archive']['error'];
		if (!$context['error_upload'] && $archive && $archive != 'none' && is_uploaded_file($archive)) { // Le fichier a ��upload�			
	...
	/**
	 * R�nitialisation de la table des objets
	 */
...
//D�inition de la LOOP sur les fichiers d'import d�ect�
...
// D�inition de la loop pour les fichier du ME
