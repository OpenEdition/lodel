...
class XMLLogic extends Logic {
  ...
  function generateXMLAction (&$context, &$error) {
  	...
  	global $db;
  	$id=$context['id'];
  	//verif if the given id is OK
  	$sql = lq("SELECT id,class from #_TP_objects WHERE id='$id'");
#echo "sql=$sql";
  	$row = $db->getRow($sql);
  	if (!$row || $row['class']!='entities') {//if the object is not an entity do not generate XML
  		header ("Location: not-found.html"); return;
  	}
  	$sql = lq("SELECT e.id,t.type,t.class FROM #_TP_entities as e, #_TP_types as t WHERE e.id='$id' AND e.idtype=t.id");
  	$row = $db->getRow($sql);
  	if (!$row) {
  		die("Error. Type and Class unknown for this entity");
  	}
  	...
  	if ($context['valid']) {
    	...
    	$ret = $client->validateXML("","");
  		...
  		return "_ok";
			...
			//TODO : je changerais entite par le nom de la classe et eventuellement l'id par l'identifier
		  download ("", "entite-$id.xml", $contents);
  	}
  	return "_ok";
  }
  /**
