...
//
  ...
  $querybase="SELECT e3.*,t3.type,t3.classe FROM $GLOBALS[tp]entites as e0 INNER JOIN $GLOBALS[tp]types as t0 ON e0.idtype=t0.id, $GLOBALS[tp]entites as e3 INNER JOIN $GLOBALS[tp]types as t3 ON e3.idtype=t3.id WHERE e0.id='$id' AND e3.idparent=e0.idparent AND e3.statut>$statutmin AND e0.statut>$statutmin AND e3.ordre".$compare."e0.ordre AND ".mysql_not_xor("t0.classe='publications'","t3.classe='publications'")." AND t3.type NOT LIKE 'documentannexe-%' ORDER BY e3.ordre ".$sort." LIMIT 0,1";
  ...
  do {
    ...
    $result=mysql_query ("SELECT e3.*,t3.type,t3.classe FROM $GLOBALS[tp]entites as e0 INNER JOIN $GLOBALS[tp]types as t0 ON e0.idtype=t0.id, $GLOBALS[tp]entites as e1, $GLOBALS[tp]entites as e2, $GLOBALS[tp]entites as e3 INNER JOIN $GLOBALS[tp]types as t3 ON e3.idtype=t3.id  WHERE e0.id='$id' AND e1.id=e0.idparent AND e2.idparent=e1.idparent AND e3.idparent=e2.id AND e2.ordre".$compare."e1.ordre AND e1.idtype IN ('$types') AND e2.idtype IN ('$types') AND e0.statut>$statutmin AND e1.statut>$statutmin AND e2.statut>$statutmin AND e3.statut>$statutmin AND  ".mysql_not_xor("t0.classe='publications'","t3.classe='publications'")." AND t3.type NOT LIKE 'documentannexe-%' ORDER BY e2.ordre ".$sort.", e3.ordre ".$sort." LIMIT 0,1") or die (mysql_error());
