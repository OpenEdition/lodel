...
{
	// Dtermine le charset a fournir au navigateur
	...
	if (!$charset)	{
		...
		if ($_SERVER["HTTP_ACCEPT_CHARSET"]) {
			...
			return "iso-8859-1"; // Si on a rien trouv on renvoie de l'iso
	}	else
		return $charset;
}
...
$context = array ("version" => $GLOBALS['version'], "shareurl" => $GLOBALS['shareurl'], "extensionscripts" => $GLOBALS['extensionscripts'], "currenturl" => "http://".$_SERVER['SERVER_NAME']. ($_SERVER['SERVER_PORT'] ? ":".$_SERVER['SERVER_PORT'] : "").$_SERVER['REQUEST_URI'],
	#"database"=>defined("DATABASE") ? DATABASE : $GLOBALS['database'],
	"siteroot" => defined("SITEROOT") ? SITEROOT : "", "site" => $site, "charset" => getacceptedcharset($charset), "langcache" => array ()); // tres important d'initialiser le context.
