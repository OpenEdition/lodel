...
// 		}
	...
	protected function _makeMask(&$context, &$error)
	{
		...
		if(isset($context['mask_regexp'])) {
		    ...
		    $mask = $context['mask']['user'] = preg_replace_callback("/^(.)(.*)(\\1)([msieDASuUXxJ]*)?$/s",function ($text){return($text[1].$text[2].$text[1].str_replace('e','',"$text[4]"));},$mask);
		    
		    // preg_replace('/^(.)(.*)(\\1)([msieDASuUXxJ]*)?$/e', "'\\1'.\\2.'\\1'.str_replace('e', '', \"\\4\")", $mask);
