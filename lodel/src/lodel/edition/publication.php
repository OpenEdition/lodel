...
if ($parent) { die("veuillez changer l'appel, parent =>idparent"); }
...
//
  ...
  $result=mysql_query("SELECT $GLOBALS[tp]publications.*, $GLOBALS[tp]entites.*, type,tplcreation  FROM  $GLOBALS[publicationstypesjoin] WHERE $GLOBALS[tp]entites.id='$id'  $critere") or die (mysql_error());
  $context[entite]=mysql_fetch_assoc($result);
  // transfer dans le context
  foreach(array("idparent","type","idtype","tplcreation","nom") as $k) {
    $context[$k]=$context[entite][$k]; unset($context[entite][$k]);
  }
